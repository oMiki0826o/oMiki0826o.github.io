...
所有 `config/*.json` 檔案都可以寫註解（嚴格來說是 JSONC 格式），
`js/loader.js` 的 `fetchJSON()` 會在解析前自動移除註解。

撰寫規則只有一個：**註解必須整行獨立**，不要寫在資料的同一行後面。
...
```jsonc
{
  ...
  "url": "https://example.com", // ❌ 不要這樣寫，行尾的網址含有 //
                                 //    會跟註解規則衝突，無法保證解析正確
}
```
...
淺色棉花糖漸層背景（奶霜白 → 粉霧 → 薰衣草藍）搭配流螢主題的暖金色
點綴，背景有緩慢飄浮的螢火光點，游標換成跟隨拖曳的愛心圖示。
專案／文章卡片採用類似 lit.link 的呈現方式：大圖在上、一張卡片一張
卡片由上往下排列，而不是多欄網格。
