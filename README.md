...
大頭貼（Hero）、導覽列圖示、favicon 三者各自獨立（`site.json` 的
`avatar`／`navIcon`／`favicon`），可以放三張不同的圖片。前兩者是網站
自己畫出來的 `<img>`，已用 CSS 裁成圓形；favicon 顯示在瀏覽器分頁上，
那個位置的外觀由瀏覽器自己的介面決定，網站程式碼無法從外部把它裁圓——
想要視覺上是圓的，需要提供本身就已經裁切成圓形、四角透明的圖片檔案。

字型網址集中定義在 `css/variables.css` 最上方的 `@import`，不再像
之前那樣每個 HTML 頁面各自貼一份重複的字型連結。換字型只需要改
這一個檔案，詳見 `config/HOW_TO.md` 的「更改全站字型」章節。
